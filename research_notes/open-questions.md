...
**Status 2026-09-20 (finding #24):** all 60 referenced species are fitted and stored in `raw/MegaCandyMaterialFloats.json` (estimates, with per-float intervals); `_BrightnessCurve` ~0.15 and `_GlassTint` ~0.1 look like real constants, `_GlowIntensity` ~0.77 / `_Stop4` ~0.84 probably, `_Stop1-3` vary per species, `_GlowColorIntensity` cannot be identified from images. Still open: 5 species (Aerodactyl, Gallade, Ampharos, Alakazam, Falinks) stay above RMSE 15 - not a search failure. Replacing ten references (Blastoise, Steelix, Malamar, Staraptor, Garchomp, Venusaur, Beedrill, Gengar, Lopunny, Pidgeot) with cleaned in-game screenshots took each of them to the noise floor (findings #25-26, #28), so the likely fix is better references, not a better model; the old gold references wanted a much lower curve because they are ImageMagick thumbnails of older builds (finding #28). Seven such thumbnails remain (Altaria, Houndoom, Manectric, Charizard, Abomasnow, Ampharos, Gyarados). Several fits push `_GlowColorIntensity` to its bound (1) and want more (1.03-1.5 when the range is widened), which hints the outline term saturates or something in alpha / the G channel is mis-modelled.  Until then, **fit these nine floats per species
